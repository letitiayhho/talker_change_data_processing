...
get_subject_abs_averages <- function(data) {
  channel_labels <- paste("X", as.character(1:128), sep = "")
  data <- data %>%
    group_by(subject_number) %>%
    summarise_at(vars(all_of(channel_labels)),  abs) %>%
    summarise_at(vars(all_of(channel_labels)), mean) %>%
    select(all_of(channel_labels))
  return(data)
}
...
```{r}
# overall_abs <- get_subject_abs_averages(xcorr)
# overall <- get_subject_averages(xcorr)
# overall_w <- get_one_sample_wilcoxon_for_each_channel(overall, alt = ALT)
overall_q <- get_ljungbox(overall_abs)
# overall_q <- get_ljungbox(overall)

# Creating a table to display all the p-values
spacer <- matrix(" ", 128, 1)
overall_ps <- data.frame(channels, 'p'= overall_q$p)
