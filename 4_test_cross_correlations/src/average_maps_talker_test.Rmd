...
Shist <- hist(unlist(S))
plot(Thist, col = c1)
plot(Shist, col = c2, add = TRUE)

# Plot p values
Tphist <- hist(T_w$p)
Sphist <- hist(S_w$p)
plot(Tphist, col = c1)
plot(Sphist, col = c2, add = TRUE)

# Plot test statistics
Twhist <- hist(T_w$w)
Swhist <- hist(S_w$w)
plot(Twhist, col = c1)
plot(Swhist, col = c2, add = TRUE)
```
###One-sample tests
...
```{r}
same <- data.frame(S_w, coordinates)

plot <- ggplot() +
  geom_point(data = same, aes(x = x, y = y, 
                                    size = 1/p, 
                                    alpha = 0.5,
                                    stroke = 0)) +
  geom_point(data = same, aes(x = x, y = y, 
...
```{r}
different <- data.frame(T_w, coordinates)

plot <- ggplot() +
  geom_point(data = different, aes(x = x, y = y, 
                                    size = 1/p, 
                                    alpha = 0.5,
                                    stroke = 0)) +
  geom_point(data = different, aes(x = x, y = y, 
...
```{r}
talker_w_greater <- get_two_sample_wilcoxon(S, T, "greater")
talker_w_less <- get_two_sample_wilcoxon(S, T, "less")
talker_w <- get_two_sample_wilcoxon(S, T, "two.sided")
```

```{r}
talker_greater <- data.frame(talker_w_greater, coordinates)

plot <- ggplot() +
  geom_point(data = talker_greater, aes(x = x, y = y, 
                                    size = 1/p, 
                                    alpha = 0.5,
                                    stroke = 0)) +
  geom_point(data = talker_greater, aes(x = x, y = y, 
...
```
```{r}
talker_less <- data.frame(talker_w_less, coordinates)

plot <- ggplot() +
  geom_point(data = talker_less, aes(x = x, y = y, 
                                    size = 1/p, 
                                    alpha = 0.5,
                                    stroke = 0)) +
  geom_point(data = talker_less, aes(x = x, y = y, 
...
```
```{r}
talker <- data.frame(talker_w, coordinates)

plot <- ggplot() +
  geom_point(data = talker, aes(x = x, y = y, 
                                    size = 1/p, 
                                    alpha = 0.5,
                                    stroke = 0)) +
  geom_point(data = talker, aes(x = x, y = y, 
