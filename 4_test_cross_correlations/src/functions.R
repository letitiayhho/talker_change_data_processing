...
get_one_sample_wilcoxon <- function(data) {
  w <- apply(data, MARGIN = 2, function(channel) {wilcox.test(channel, exact = TRUE)$statistic})
  p <- apply(data, MARGIN = 2, function(channel) {wilcox.test(channel, exact = TRUE)$p.value})
