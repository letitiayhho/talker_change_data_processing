...
p <- ggplot(overall_w, aes(x = w_adjusted)) +
    ...
    xlab("w")
p 
ggsave(filename = "../figs/ws_overall_poster.png", plot = p, width = 6, height = 2.5)
sum(overall_w$p < 0.05)
```
...
df <- get_sig_xcorrs_df('1', 'talker', S_sig) %>%
  rbind(get_sig_xcorrs_df('2', 'talker', T_sig)) %>%
  rbind(get_sig_xcorrs_df('1', 'meaning', M_sig)) %>%
  rbind(get_sig_xcorrs_df('2', 'meaning', N_sig)) %>%
  rbind(get_sig_xcorrs_df('1', 'constraint', L_sig)) %>%
  rbind(get_sig_xcorrs_df('2', 'constraint', H_sig))

# New facet label names for condition variable
condition.labs <- c("S", "T", "M", "N", "L", "H")

p <- ggplot(df, aes(x = condition, y = sig_xcorr, fill = group)) +
  geom_boxplot() +
...
# p
# getwd()
ggsave(p, filename = '4_test_cross_correlations/figs/xcorrs_for_sig_channels_poster_3.png', width = 10, height = 5)
```
