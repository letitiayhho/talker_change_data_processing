...
plot_ws <- function(w1, w2, labs, c, fname) {
  df_long <- data.frame(w1 = unlist(w1), w2 = unlist(w2)) %>%
    pivot_longer(cols = c("w1", "w2"),
                 names_to = "condition",
                 values_to = "W")
  p <- ggplot(df_long, aes(x = W, fill = condition, color = condition)) +
...
get_sig_xcorrs <- function(cond, group, xcorr, p) { # get xcorrs from sig channels
  sig_xcorrs <- as.matrix(xcorr) %>%
   .[, which(p < 0.05)] %>% 
    abs() %>%
    rowMeans() %>% # comparing channels, rowMeans() compares sub
    remove_outliers()
  cond_col <- rep(cond, length(sig_xcorrs))
  group_col <- rep(group, length(sig_xcorrs))
  df <- data.frame(cond, xcorrs = sig_xcorrs, group)
  return(df)
}
...
# get_sig_xcorr_stats <- function(cond, group, xcorr, p) {
#   sig_xcorrs <- get_sig_xcorrs(cond, xcorr, p)$xcorrs
#   print(sig_xcorrs)
#   xcorr <- mean(sig_xcorrs)
#   sd <- sd(sig_xcorrs)
#   return(data.frame('condition' = cond,
#                     'group' = group,
#                     'xcorr' = xcorr,
#                     'sd' = sd))
# }

# df <- get_sig_xcorr_stats('S', 'talker', S, S_w$p) %>%
#   rbind(get_sig_xcorr_stats('T', 'talker', T, T_w$p)) %>%
#   rbind(get_sig_xcorr_stats('M', 'meaning', M, M_w$p)) %>%
#   rbind(get_sig_xcorr_stats('N', 'meaning', N, N_w$p)) %>%
#   rbind(get_sig_xcorr_stats('L', 'constraint', L, L_w$p)) %>%
#   rbind(get_sig_xcorr_stats('H', 'constraint', H, H_w$p))


df <- get_sig_xcorrs('S', 'talker', S, S_w$p) %>%
  rbind(get_sig_xcorrs('T', 'talker', T, T_w$p)) %>%
  rbind(get_sig_xcorrs('M', 'meaning', M, M_w$p)) %>%
  rbind(get_sig_xcorrs('N', 'meaning', N, N_w$p)) %>%
  rbind(get_sig_xcorrs('L', 'constraint', L, L_w$p)) %>%
  rbind(get_sig_xcorrs('H', 'constraint', H, H_w$p))

p <- ggplot(df, aes(x = cond, y = xcorrs, fill = group)) + 
  geom_boxplot()
  # geom_boxplot(stat="identity", position=position_dodge()) +
  # geom_errorbar(aes(ymin=xcorr-sd, ymax=xcorr+sd), width=.2,
                 # position=position_dodge(.9)) +
  # geom_signif(comparisons = list(c("S", "T")), annotations = '***') +
  # ylab("xcorr") +
  # scale_fill_brewer(palette="Blues")
p
ggsave(p, filename = '4_test_cross_correlations/figs/xcorrs_for_sig_channels.png', width = 6, height = 3.5)
```


```{r}
# ANOVA between levels 

# df <- get_sig_xcorrs('S', S, S_w$p) %>%
#   rbind(get_sig_xcorrs('T', T, T_w$p)) %>%
#   rbind(get_sig_xcorrs('M', M, M_w$p)) %>%
#   rbind(get_sig_xcorrs('N', N, N_w$p)) %>%
#   rbind(get_sig_xcorrs('L', L, L_w$p)) %>%
#   rbind(get_sig_xcorrs('H', H, H_w$p))

one.way <- aov(xcorrs ~ cond, data = df)
TukeyHSD(one.way)

# ANOVA between conditions

df <- get_sig_xcorrs('talker', S, S_w$p) %>%        
  rbind(get_sig_xcorrs('talker', T, T_w$p)) %>%
  rbind(get_sig_xcorrs('meaning', M, M_w$p)) %>%
  rbind(get_sig_xcorrs('meaning', N, N_w$p)) %>%
  rbind(get_sig_xcorrs('constraint', L, L_w$p)) %>%
  rbind(get_sig_xcorrs('constraint', H, H_w$p))
