...
```{r}
load("data/5_rms/simple_linear_models_means.RDa")
load("data/5_rms/simple_linear_models_sds.RDa")
...
```{r}
load("data/5_rms/multilevel_models_means.RDa")
load("data/5_rms/multilevel_models_sds.RDa")
```
...
for (i in 1:128) {
  ...
  if (i %in% talker_normalization_channels & z_results$z > 0) {
    diff_df$z_sig[i] <- z_results$z
    diff_df$p_sig[i] <- z_results$p
  } else if (i %in% talker_normalization_channels & z_results$z < 0) {
    diff_df$z_sig_neg[i] <- z_results$z
    diff_df$p_sig_neg[i] <- z_results$p    
  } else if (z_results$z > 0) {
    diff_df$z_unsig[i] <- z_results$z
    diff_df$p_unsig[i] <- z_results$p    
  } else {
    diff_df$z_unsig_neg[i] <- z_results$z
    diff_df$p_unsig_neg[i] <- z_results$p
  }
}
```
...
ggplot() +
  ...
  geom_point(data = diff_df, 
             ...
             aes(x = x, 
                 y = y,
                 size = z_unsig_neg*100,
                 alpha = log(1/p_unsig_neg),
                 stroke = 0)) +
  geom_point(data = diff_df,
             aes(x = 1000+x,
                 y = y,
...
ggplot(data = post, aes(z_score, colour = type)) +
  geom_freqpoly()
```
