...
```{r, message=FALSE, echo=FALSE}
# # Cross-correlation
# xcorr_same_talker <- get_one_sample_t(filter(xcorr, talker == 'S'))
# xcorr_diff_talker <- get_one_sample_t(filter(xcorr, talker == 'T'))
# xcorr_meaningful <- get_one_sample_t(filter(xcorr, meaning == 'M'))
# xcorr_nonsense <- get_one_sample_t(filter(xcorr, meaning == 'N'))
# xcorr_general <- get_one_sample_t(filter(xcorr, constraint == 'G'))
# xcorr_specific <- get_one_sample_t(filter(xcorr, constraint == 'S'))
# 
# # Convolution
# conv_same_talker <- get_one_sample_t(filter(conv, talker == 'S'))
# conv_diff_talker <- get_one_sample_t(filter(conv, talker == 'T'))
# conv_meaningful <- get_one_sample_t(filter(conv, meaning == 'M'))
# conv_nonsense <- get_one_sample_t(filter(conv, meaning == 'N'))
# conv_general <- get_one_sample_t(filter(conv, constraint == 'G'))
# conv_specific <- get_one_sample_t(filter(conv, constraint == 'S'))
# 
# # Create frequency table
# condition_freq_table <- data.frame("talker S" = c(sum(xcorr_same_talker$p < 0.1), sum(conv_same_talker$p < 0.1)),
#                                    "talker T" = c(sum(xcorr_diff_talker$p < 0.1), sum(conv_diff_talker$p < 0.1)),
#                                    "meaning M" = c(sum(xcorr_meaningful$p < 0.1), sum(conv_meaningful$p < 0.1)),
#                                    "meaning N" = c(sum(xcorr_nonsense$p < 0.1), sum(conv_nonsense$p < 0.1)),
#                                    "constraint S" = c(sum(xcorr_specific$p < 0.1), sum(conv_specific$p < 0.1)),
#                                    "constraint G" = c(sum(xcorr_general$p < 0.1), sum(conv_general$p < 0.1))) 
# row.names(condition_freq_table) <- c("cross correlation", "convolution")
# kable(condition_freq_table) %>%
