...
```{r}
overall <- read.csv('9_N1_RMS/data/overall_lin_reg.csv') %>%
  bind_cols(coords)
plot <- recode_ps(overall) %>%
  get_map('Overall')
ggsave(plot = plot, filename = '9_N1_RMS/figs/overall_lin_reg.png', width = 6, height = 5)
```
