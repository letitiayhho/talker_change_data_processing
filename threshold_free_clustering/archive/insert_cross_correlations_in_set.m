function [] = insert_cross_correlations_in_set()
    %% 
    ...
    for subject_number = subjects
        fprintf(1, strcat("Inserting cross_correlations from .set to fieldtrip format for subject #", subject_number, "\n"))
        ...
        % Load the .mat file
        cross_correlations_fp = fullfile('threshold_free_clustering/data', subject_number, 'cross_correlations.mat')
        load(cross_correlations_fp);

        % Replace EEG.data with data used in my analysis
        EEG.data = cross_correlations;

        % Update the ALLEEG every time you modify the EEG structur
        [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);

        % Save the new EEG data struct as .set
        set_fp = fullfile('threshold_free_clustering/data', subject_number, 'cross_correlations.set')
        save(set_fp, 'EEG', '-mat')
