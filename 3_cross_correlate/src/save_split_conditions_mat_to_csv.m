...
for i = 1:numel(subject_numbers)
    ...
    stim_order_mat_path = strcat("3_cross_correlate/data/", num2str(subject_numbers(i)), "/split_conditions.mat")
    stim_order_csv_path = strcat("3_cross_correlate/data/", num2str(subject_numbers(i)), "/split_conditions.csv")

    stim_order_table = load(stim_order_mat_path).split_conditions;
