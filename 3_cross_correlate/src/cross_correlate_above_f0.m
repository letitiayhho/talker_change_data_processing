...
arguments
    ...
    addpath(fullfile('3_cross_correlate/data/stim/above_f0')) % add audio stimuli directory to path
...
%              lag(j, i) = lags(I);
    ...
    function [] = save_xcorr(subject_number, condition, stim_order, data, stat)
        ...
        % Save
        fp = fullfile('3_cross_correlate/data', subject_number, [stat, '_above_f0_normalized.mat']);
        fprintf(1, ['\nWriting data to /', fp, '\n'])
        save(fp, 'data_frame')
    end
